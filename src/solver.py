...
class Solver():
    ...
    def __init__(self, config, paras):
        ...
        if self.exp_name is None:
            self.exp_name = '_'.join(
                [paras.config.split('/')[-1].replace('.yaml', ''), 'sd'+str(paras.seed)])
    ...
    def progress(self, msg):
        ...
        if self.paras.verbose:
            print('[{}] {}'.format(human_format(
                self.step), msg).ljust(100), end='\r')
...
class Trainer(Solver):
    ...
    def set_model(self):
        ...
        # Model
        embedding_config = self.config["model"].pop("embedding", None)
        embedding_filepath = embedding_config.get("filepath", None)
        embeddings = None
        if embedding_filepath is not None:
            embeddings = util.load_embedding(
                self.tokenizer, embedding_filepath)
        embedding_loss = embedding_config.get(
            "embedding_loss", "adaptive_cosine_softmax")
        self.config["model"]["decoder"].update(
            {
                "embeddings": embeddings,
                "embedding_loss": embedding_loss
            })

        self.asr_model = ASR(self.feat_dim, self.vocab_size,
                             **self.config['model']).to(self.device)
        for m in self.asr_model.create_msg():
            self.verbose(m)
        if embeddings is not None:
            self.asr_model.decoder.target_embeddings.weight.copy_(
        ...
        self.optimizer = Optimizer(
            self.asr_model.parameters(), **self.config['hparas'])
        ...
        while self.step < self.max_step:
            for data in self.tr_set:
                ...
                ctc_output, encode_len, att_output, att_align = \
                    self.asr_model(feat, feat_len, max(txt_len),
                                   tf_rate=tf_rate, teacher=txt)
                ...
                if att_output is not None:
                    ...
                    att_loss = torch.mean(torch.sum(att_loss.view(
                        b, t), dim=-1)/torch.sum(txt != 0, dim=-1).float())
    ...
    def validate(self):
        ...
        for i, data in enumerate(self.dv_set):
            ...
            with torch.no_grad():
                ctc_output, encode_len, att_output, att_align = \
                    self.asr_model(feat, feat_len, max(
                        txt_len)+ADDITIONAL_DEV_STEP)
        ...
        for i in range(min(len(txt), DEV_N_EXAMPLE)):
            ...
            if att_output is not None:
                self.write_log('att_align{}'.format(
                    i), att_align[i, 0, :, :].cpu().unsqueeze(0))
                self.write_log('att_text{}'.format(i), self.tokenizer.decode(
                    att_output[i].argmax(dim=-1).tolist()))
            if ctc_output is not None:
                self.write_log('ctc_text{}'.format(i), self.tokenizer.decode(
    ...
    def save_checkpoint(self, f_name, score):
        ...
        self.verbose("Saved step {} checkpoint (wer = {:.2f}%) and status @ {}".format(
            human_format(self.step), score*100, ckpt_path))
...
class LMTrainer(Solver):
    ...
    def set_model(self):
        ...
        self.rnnlm = RNNLM(self.vocab_size, **
                           self.config['model']).to(self.device)
        for m in self.rnnlm.create_msg():
            self.verbose(m)
        # Losses
        self.seq_loss = torch.nn.CrossEntropyLoss(ignore_index=0)
        # Optimizer
        self.optimizer = Optimizer(
            self.rnnlm.parameters(), **self.config['hparas'])
        # ToDo: load pre-trained model
        if self.paras.load:
            raise NotImplementedError
    ...
    def exec(self):
        ...
        while self.step < self.max_step:
            for data in self.tr_set:
                ...
                lm_loss.backward()
                grad_norm = torch.nn.utils.clip_grad_norm_(
                    self.rnnlm.parameters(), GRAD_CLIP)
                if math.isnan(grad_norm):
                    self.verbose(
                        'Error : grad norm is NaN @ step '+str(self.step))
                else:
    ...
    def save_checkpoint(self, f_name, score):
        ...
        torch.save(full_dict, ckpt_path)
        self.verbose(
            "Saved checkpoint (ppx = {:.2f}) and status @ {}".format(score, ckpt_path))
